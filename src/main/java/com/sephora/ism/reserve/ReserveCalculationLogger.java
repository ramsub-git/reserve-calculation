...
public class ReserveCalculationLogger {
    ...
    public static void logInitialState(ReserveCalcContext context) {
        ...
        if (context.getInitialValueWrapper() != null) {
            ...
            Map<ReserveField.FieldCategory, List<Map.Entry<ReserveField, BigDecimal>>> grouped =
    ...
    private static void logConstraintCalculation(ReserveField constraintField, ReserveCalcContext context) {
        ...
        if (baseField != null) {
            ...
            logger.info("  Available inventory: %s%n",
                    formatValue(getAvailableInventory(context, constraintField)));
            logger.info("  Constraint result: %s%n", formatValue(constraintValue));

            if (constraintValue.compareTo(BigDecimal.ZERO) > 0) {
                logger.info("  %sINVENTORY CONSTRAINED!%s Cannot fulfill full amount of %s%n",
                        RED, RESET, formatValue(baseValue));
            }
        }
    }
    ...
    private static void logRunningInventory(ReserveCalcContext context) {
        ...
        for (String[] deduction : deductions) {
            ...
            if (amount.compareTo(BigDecimal.ZERO) > 0) {
                ...
                logger.info("  %-30s : -%9s = %10s%n",
                        "After " + name, formatValue(amount), formatValue(running));
            }
        }
        ...
        if (!running.equals(uncomafs)) {
            logger.info("  %sDISCREPANCY: %s%s%n",
                    RED + BOLD, formatValue(running.subtract(uncomafs)), RESET);
        }
        ...
        ReserveField[][] reserves = {
                {ReserveField.DOTHRY, ReserveField.DOTHRYX},
                {ReserveField.DOTHRN, ReserveField.DOTHRNX},
                {ReserveField.RETHRY, ReserveField.RETHRYX},
                {ReserveField.RETHRN, ReserveField.RETHRNX},
                {ReserveField.HLDHR, ReserveField.HLDHRX},
                {ReserveField.DOTRSV, ReserveField.DOTRSVX},
                {ReserveField.RETRSV, ReserveField.RETRSVX},
                {ReserveField.AOUTBV, ReserveField.AOUTBVX},
        ...
        for (ReserveField[] pair : reserves) {
            ...
            if (value.compareTo(BigDecimal.ZERO) > 0) {
                ...
                logger.info("  %-30s : -%9s = %s%10s%s%n",
                        "After " + pair[0], formatValue(value), color, formatValue(running), RESET);
            }
        }

        logger.info("  " + "=".repeat(45));
        logger.info("  %s%-30s : %10s%s%n",
                running.compareTo(BigDecimal.ZERO) < 0 ? RED + BOLD : GREEN,
     ...
     */
    private static void addDeduction(List<String[]> deductions, String name,
                                     ReserveCalcContext context, ReserveField base, ReserveField constraint) {
    ...
    private static void performValidationChecks(ReserveCalcContext context) {
        ...
        BigDecimal initAfs = context.get(ReserveField.INITAFS);
        BigDecimal totalAllocated = context.get(ReserveField.COMMITTED)
                .add(context.get(ReserveField.DOTATS))
                .add(context.get(ReserveField.RETAILATS));

        if (totalAllocated.compareTo(initAfs) > 0) {
            logger.info("✗ FAILED: Total allocated ({}) > INITAFS ({})",
                    formatValue(totalAllocated), formatValue(initAfs));
            allPassed = false;
        } else {
