...
// 1. In ReserveCalculationEngine.calculate():
/*
public void calculate(ReserveCalcContext context) {
    ...
    if (!enginePreCheck.test(context)) {
        throw new IllegalStateException("Engine pre-check failed");
    }
    ...
    if (!enginePostCheck.test(context)) {
        throw new IllegalStateException("Engine post-check failed");
    }
}
*/

// 2. In CalculationStep.compute() for debugging specific calculations:
/*
@Override
protected BigDecimal compute(ReserveCalcContext context) {
    Map<String, BigDecimal> inputs = new LinkedHashMap<>();
    for (String field : getDependencyFields()) {
        inputs.put(field, context.get(field));
    }
...
// 3. In ReserveCalculationController for comparison with Excel:
/*
@GetMapping("/test")
